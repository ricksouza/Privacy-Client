...
//           "of 5 and 7...\n", serverUrl, methodName);
    ...
    printf("Authentication user: rick pass: 123456 \n");

    /* Make the remote procedure call */
//    resultP = xmlrpc_client_call(&env, serverUrl, methodName,
//                                 "(ii)", (xmlrpc_int32) 120, (xmlrpc_int32) 500);

    resultP = xmlrpc_client_call(&env, serverUrl, "authentication", "(ss)", "rick", "123456");
...
//    printf("The sum is %d\n", sum);
     ...
     * Ler resposta
     */
    //xmlrpc_read_string(&env, result)
    xmlrpc_read_int(&env, resultP, &rt);
    dieIfFaultOccurred(&env);
    if(rt == 1)
    	printf("The authentication was OK ! \n");
    else
    	printf("Authentication failled! \n");
